...
if ($useFormSetup && (float) DOL_VERSION >= 15) {
	...
	// Setup conf DOLITRASHCAN_MYPARAM1 as a simple string input
	$item = $formSetup->newItem('DOLITRASHCAN_MYPARAM1');

	// Setup conf DOLITRASHCAN_MYPARAM1 as a simple textarea input but we replace the text of field title
	$item = $formSetup->newItem('DOLITRASHCAN_MYPARAM2');
	$item->nameText = $item->getNameText().' more html text ';

	// Setup conf DOLITRASHCAN_MYPARAM3
	$item = $formSetup->newItem('DOLITRASHCAN_MYPARAM3');
	$item->setAsThirdpartyType();

	// Setup conf DOLITRASHCAN_MYPARAM4 : exemple of quick define write style
	$formSetup->newItem('DOLITRASHCAN_MYPARAM4')->setAsYesNo();

	// Setup conf DOLITRASHCAN_MYPARAM5
	$formSetup->newItem('DOLITRASHCAN_MYPARAM5')->setAsEmailTemplate('thirdparty');

	// Setup conf DOLITRASHCAN_MYPARAM6
	$formSetup->newItem('DOLITRASHCAN_MYPARAM6')->setAsSecureKey()->enabled = 0; // disabled

	// Setup conf DOLITRASHCAN_MYPARAM7
	$formSetup->newItem('DOLITRASHCAN_MYPARAM7')->setAsProduct();
	*/
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/dolitrashcan/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'DOLITRASHCAN_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'DOLITRASHCAN_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'DOLITRASHCAN_'.strtoupper($myTmpObjectKey).'_ADDON';
