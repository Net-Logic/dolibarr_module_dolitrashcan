...
/**
 * \file    test/phpunit/DoliTrashCanFunctionalTest.php
...
/**
 * Class DoliTrashCanFunctionalTest
 *
 ...
 *
 * @package Testdolitrashcan
 */
class DoliTrashCanFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modDoliTrashCan')->click();
		} else {
			// Disable the module
			$this->byHref('modDoliTrashCan')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/dolitrashcan/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('dolitrashcan/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/dolitrashcan/admin/about.php');
		$this->authenticate();
		return $this->assertContains('dolitrashcan/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('dolitrashcanwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modDoliTrashCan_DoliTrashCanTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modDoliTrashCan_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
